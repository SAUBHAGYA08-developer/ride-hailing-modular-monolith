...
check "ride assigned"      "DRIVER_ASSIGNED" "$(echo "$RIDE_JSON" | jget data.status)"
check "driver attached"    "true"            "$([ -n "$(echo "$RIDE_JSON" | jget data.driverId)" ] && echo true || echo false)"
...
# The winning driver is whoever matching picked, so start as that driver.
WINNER=$(echo "$RIDE_JSON" | jget data.driverId)
WTOKEN=$DRIVER; [ "$WINNER" = "2" ] && WTOKEN=$DRIVER2
